...
public class RingSystem_EntitySpawner : MonoBehaviour
{
    ...
    Entity GetNewEntity(EntityManager entityManager, int index, int totalEntityCount, out int entityIndex)
    {
        ...
        if (v <= 100 && v > 90) entityIndex = 8;
        if (v <= 90 && v > 80) entityIndex = 7;
        if (v <= 80 && v > 70) entityIndex = 6;
        if (v <= 70 && v > 60) entityIndex = 5;
        if (v <= 60 && v > 50) entityIndex = 4;
        if (v <= 50 && v > 40) entityIndex = 3;
        if (v <= 40 && v > 30) entityIndex = 2;
        if (v <= 30 && v > 20) entityIndex = 1;
        if (v <= 20) entityIndex = 0;
